...
void main(int argc, char **argv)
{
	...
	*/
	WriteChar(nPortFD,'\033');
	WriteChar(nPortFD,'\005');
	/*
	WriteChar(nPortFD,'C');
	WriteChar(nPortFD,'A');
	WriteChar(nPortFD,'\r'); WriteChar(nPortFD,'\n');
	*/
	/*
	printf("Messaggio Spedito [%d]\n",nNumCar);
	*/
	...
	while(1){
		...
		for(nIndex=0;nIndex<strlen(szRcvBuffer);nIndex++){
			printf("[%02.2x]",szRcvBuffer[nIndex]&~(0x80));
		}
		printf("\r\n");
	}
