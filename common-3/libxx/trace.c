...
/*****************************************************************************
  FUNCTION: int open_trace(char *filename, int trace_len)
  ...
  RETURNS:   0 - OK
            !0 - UNIX error number
 ****************************************************************************/
int open_trace (filename, trace_len)
	char *filename;
	int trace_len;
{
	char cch;
	...
	if ((trace.fp = fopen(trace_filename, "a+")) == (FILE *)NULL) {
		sprintf(err_msg, "cannot open trace file '%s' (errno: %d)", trace_filename, errno);	
