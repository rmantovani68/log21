#ifndef __MAINWIN_H
#define __MAINWIN_H

#include <gtk/gtk.h>

#include <proc_list.h>
#include "mainstruct.h"
#include "mainapp.h"


#define MAIN_WINDOW_TYPE (main_window_get_type ())
G_DECLARE_FINAL_TYPE (MainWindow, main_window, MAIN, WINDOW, GtkApplicationWindow)


struct _MainWindow
{
    GtkApplicationWindow parent;
};


struct _MainWindowPrivate
{
    MainApp *app;

    gint nFaseOld;
    gint nFase;

    GSettings *settings;
    WindowSizeInfo *wsi;

    gint current_width;
    gint current_height;
    gboolean is_maximized;
    gboolean is_fullscreen;

    GdkScreen *screen;
    GtkCssProvider *provider;

    gint nPID;
    gchar *pszAppName;
    gchar *pszAppTitle;
    gchar *pszAppClass;
    gchar *pszCfgFileName;

    LINEASPEDIZIONESTRUCT Linea;
    BANCALESTRUCT Bancale;
    COLLOSTRUCT Collo;

    GtkWidget *txt_msgs;
    GtkWidget *lb_msg;
    GtkWidget *lb_linee_titolo;
    GtkWidget *lb_bancali_titolo;
    GtkWidget *lb_colli_titolo;
    GtkWidget *lb_lntpspe;
    GtkWidget *lb_lncdlin;
    GtkWidget *lb_key_linea;
    GtkWidget *lb_lnnmced;
    GtkWidget *lb_lnnmbnc;
    GtkWidget *lb_lnnmcll;
    GtkWidget *lb_lnpsrea;
    GtkWidget *lb_lnpspre;
    GtkWidget *lb_bntpspe;
    GtkWidget *lb_bncdlin;
    GtkWidget *lb_cdspe_tit;
    GtkWidget *lb_key_bancale;
    GtkWidget *lb_nmcll_tit;
    GtkWidget *lb_psnet_tit;
    GtkWidget *lb_bnnmcll;
    GtkWidget *lb_bnpsrea;
    GtkWidget *lb_bnpspre;
    GtkWidget *lb_barcode;
    GtkWidget *lb_cptpspe;
    GtkWidget *lb_cpcdlin;
    GtkWidget *lb_cpnmcol;
    GtkWidget *lb_cptpfor;
    GtkWidget *lb_cpnmrgh;
    GtkWidget *lb_cpnmcpe;
    GtkWidget *lb_cppsrea;
    GtkWidget *lb_cppspre;
    GtkWidget *lb_ordprog;
    GtkWidget *lb_key_collo;

    GtkWidget *entry_barcode;
    GtkWidget *sw_linee;
    GtkWidget *sw_bancali;
    GtkWidget *sw_colli;

    GtkWidget *pb_inizio_linea;
    GtkWidget *pb_fine_linea;
    GtkWidget *pb_apertura_bancale;
    GtkWidget *pb_chiusura_bancale;
    GtkWidget *pb_stampa_linea;
    GtkWidget *pb_stampa_bancale;
    GtkWidget *pb_config;
    GtkWidget *pb_select_linea;
    GtkWidget *pb_linee;
    GtkWidget *pb_exit;

};

typedef struct _MainWindowPrivate MainWindowPrivate;

MainWindow *main_window_new (MainApp *app);

MainWindowPrivate *internal_main_window_get_instance_private (MainWindow *win);


#define TREE_VIEW_LST(widget)       (GTK_TREE_VIEW(widget))
#define TREE_SELECTION_LST(widget)  (GTK_TREE_SELECTION((gtk_tree_view_get_selection(TREE_VIEW_LST(widget)))))
#define TREE_MODEL_LST(widget)      (GTK_TREE_MODEL((gtk_tree_view_get_model(TREE_VIEW_LST(widget)))))

#endif /* __MAINWIN_H */
