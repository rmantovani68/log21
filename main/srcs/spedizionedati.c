...
#define WINDOW_NAME "spedizione-dati-window"

static SpedizioneType _type;

struct _SpedizioneDatiWindow
{
    GtkDialog parent;
};

typedef struct _SpedizioneDatiWindowPrivate SpedizioneDatiWindowPrivate;

struct _SpedizioneDatiWindowPrivate
{
    SpedizioneType type;
...
G_DEFINE_TYPE_WITH_PRIVATE(SpedizioneDatiWindow, spedizione_dati, GTK_TYPE_DIALOG)
...
void SpedizioneDatiSDA(gpointer win, gboolean storico, gboolean overwrite)
{
	...
	int nROPSREA;
	int nROPSPRE;
    SpedizioneDatiWindowPrivate *priv = spedizione_dati_get_instance_private (SPEDIZIONE_DATI_WINDOW (win));
...
void SpedizioneDatiCorriere(gpointer win, gboolean storico, gboolean overwrite)
{
	...
	*/
	...
	if (DBresultStatus(DBRes) != DBRES_COMMAND_OK ){
        trace_debug_gtk("RED",TRUE,TRUE,txt_msgs, "Errore in aggiornamento ordini spediti");
	}
	DBclear(DBRes);
...
void SpedizioneDati(gpointer win,gboolean overwrite, char *szExportFileName,char *szExportRigheFileName,ep_bool_t bCambiaStato, gboolean  storico)
{
    ...
    if((fpExport=fopen(szTmpExportFileName,szAperturaFile))==NULL){
        trace_debug_gtk("RED",TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"Errore in apertura file dati [%s]",szExportFileName);
        return;
    }

    if((fpExportRighe=fopen(szTmpExportRigheFileName,szAperturaFile))==NULL){
        trace_debug_gtk("RED",TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"Errore in apertura file dati [%s]",szExportRigheFileName);
        return;
    }
    ...
    */
    ...
    if((nNumeroOrdini=DBntuples(DBResOrdini))){
        ...
        trace_debug_gtk("BLUE",TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"Lanciata Spedizione Dati (%d:Ordini) a Host",nNumeroOrdini);
        ...
        for (nIndexOrdini=0;nIndexOrdini<nNumeroOrdini;nIndexOrdini++){
            ...
            */
            if(atoi(DBgetvalue(DBResOrdini,nIndexOrdini,3))==0){
                trace_debug_gtk("RED",TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"ERRORE IN SPEDIZIONE ORDINE [%s] SEGNACOLLO NON PRESENTE !",DBgetvalue(DBResOrdini,nIndexOrdini,1));
                bERRORE=TRUE;
            }
            if(!bERRORE){
                ...
                if(bCambiaStato){
                    ...
                    */
                    ...
                    UpdateOrdine(MAIN_WINDOW(priv->main_window), szOrdProg);
        ...
        if(!bERRORE){
            trace_debug_gtk("BLUE",TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"Spediti a Host %d ordini",nNumeroOrdini);
        } else {
            trace_debug_gtk("RED", TRUE,TRUE,find_child(GTK_WIDGET(priv->main_window), "txt_msgs"),"SPEDIZIONE ANNULLATA");
        }
    }
...
void do_spedizione_dati (gpointer win, gboolean storico, gboolean overwrite)
{
    ...
    SpedizioneDatiWindowPrivate *priv;

    priv = spedizione_dati_get_instance_private (win);
...
void init_spedizione_dati(SpedizioneDatiWindow *win, gboolean storico)
{
	char szExportFile[128];
	DBresult *DBResOrdini;
    SpedizioneDatiWindowPrivate *priv = spedizione_dati_get_instance_private (win);
...
void init_spedizione_SDA(SpedizioneDatiWindow *win, gboolean storico)
{
	char szExportFile[128];
	int nTuples=0;
	DBresult *DBRes;
...
void init_spedizione_corriere(SpedizioneDatiWindow *win, gboolean storico)
{
	char szExportFile[128];
	int nTuples=0;
	DBresult *DBRes;
...
void init_spedizione_corr_agg(SpedizioneDatiWindow *win, gboolean storico)
{
	char szExportFile[128];
	int nTuples=0;
	DBresult *DBRes;
...
static void spedizione_dati_init (SpedizioneDatiWindow *win)
{
    SpedizioneDatiWindowPrivate *priv = spedizione_dati_get_instance_private (SPEDIZIONE_DATI_WINDOW (win));
...
static void spedizione_dati_dispose (GObject *object)
{
    SpedizioneDatiWindowPrivate *priv;

    priv = spedizione_dati_get_instance_private (SPEDIZIONE_DATI_WINDOW (object));

    G_OBJECT_CLASS (spedizione_dati_parent_class)->dispose (object);
}

static void spedizione_dati_class_init (SpedizioneDatiWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = spedizione_dati_dispose;
    ...
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pb_ok        );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pb_close     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pb_append    );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pb_overwrite );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_1         );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_2         );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_ordini    );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_colli     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_righe     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, lb_linee     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pr_ordini    );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pr_colli     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pr_righe     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), SpedizioneDatiWindow, pr_linee     );
}

SpedizioneDatiWindow * spedizione_dati_new (gpointer win, SpedizioneType type)
{
    _type = type;
    SpedizioneDatiWindow *w = g_object_new (SPEDIZIONE_DATI_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);

    SpedizioneDatiWindowPrivate *priv = spedizione_dati_get_instance_private (SPEDIZIONE_DATI_WINDOW (w));
