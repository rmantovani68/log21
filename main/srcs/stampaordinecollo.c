...
#define WINDOW_NAME "stampa-ordine-collo-window"

/* workaround */
static STAMPA_ORDINE_COLLO_TYPE_ENUM _type;
static gchar *_ordine=NULL;

struct _StampaOrdineColloWindow
{
    GtkDialog parent;
};

typedef struct _StampaOrdineColloWindowPrivate StampaOrdineColloWindowPrivate;

struct _StampaOrdineColloWindowPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE(StampaOrdineColloWindow, stampa_ordine_collo_window, GTK_TYPE_DIALOG)
...
static void stampa_ordine_collo_window_init (StampaOrdineColloWindow *win)
{
    gchar *titolo, *question;
    StampaOrdineColloWindowPrivate *priv;

    priv = stampa_ordine_collo_window_get_instance_private (win);
...
static void stampa_ordine_collo_window_dispose (GObject *object)
{
    StampaOrdineColloWindowPrivate *priv;

    priv = stampa_ordine_collo_window_get_instance_private (STAMPA_ORDINE_COLLO_WINDOW (object));
    ...
    G_OBJECT_CLASS (stampa_ordine_collo_window_parent_class)->dispose (object);
}

static void stampa_ordine_collo_window_class_init (StampaOrdineColloWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = stampa_ordine_collo_window_dispose;
    ...
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, cb_storico       );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, cb_stampa_su_file);
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, lb_msg           );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, lb_ordine        );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, lb_collo         );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, entry_ordine     );
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), StampaOrdineColloWindow, sb_collo         );
}
...
StampaOrdineColloWindow * stampa_ordine_collo_window_new (MainWindow *win, STAMPA_ORDINE_COLLO_TYPE_ENUM t, gchar *o)
{
    ...
    StampaOrdineColloWindow *w = g_object_new (STAMPA_ORDINE_COLLO_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);

    StampaOrdineColloWindowPrivate *priv = stampa_ordine_collo_window_get_instance_private (STAMPA_ORDINE_COLLO_WINDOW (w));
