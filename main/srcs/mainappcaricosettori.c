...
#include "mainappcaricosettori.h"
#include "mainfun.h"

// workaround perchè nella funzione do_carico_settori uso il puntatore alla main_window
static MainAppWindow *main_app_window;

struct _MainAppCaricoSettori
{
    GtkDialog parent;
};

typedef struct _MainAppCaricoSettoriPrivate MainAppCaricoSettoriPrivate;

struct _MainAppCaricoSettoriPrivate
{
    MainAppWindow *main_app_window;
    GtkWidget *frm_settori;
};

G_DEFINE_TYPE_WITH_PRIVATE(MainAppCaricoSettori, main_app_carico_settori, GTK_TYPE_DIALOG)
...
void do_carico_settori(gpointer win)
{
	...
	int nSettoreIndex;
	int nTuples;
    MainAppCaricoSettoriPrivate *priv;

    priv = main_app_carico_settori_get_instance_private (MAIN_APP_CARICO_SETTORI (win));
	...
	for(nIndex=0;nIndex<nTuples;nIndex++){
		...
		if(DBntuples(DBResUbi)){
			...
			*/
			...
			gtk_text_printf("RED",find_child(GTK_WIDGET(main_app_window), "txt_msgs"),"Prodotto [%s] non ubicato !\n", DBgetvalue(DBRes,nIndex,0));
		}
		DBFreeQueryResult(DBResUbi);
	}
...
static void main_app_carico_settori_init (MainAppCaricoSettori *win)
{
    MainAppCaricoSettoriPrivate *priv;

    priv = main_app_carico_settori_get_instance_private (win);
    ...
    init_actions(win, entries, G_N_ELEMENTS(entries), "carico_settori");
}

static void main_app_carico_settori_dispose (GObject *object)
{
    MainAppCaricoSettoriPrivate *priv;

    priv = main_app_carico_settori_get_instance_private (MAIN_APP_CARICO_SETTORI (object));

    G_OBJECT_CLASS (main_app_carico_settori_parent_class)->dispose (object);
}

static void main_app_carico_settori_class_init (MainAppCaricoSettoriClass *class)
{
    G_OBJECT_CLASS (class)->dispose = main_app_carico_settori_dispose;

    gtk_widget_class_set_template_from_resource (GTK_WIDGET_CLASS (class), "/org/easy-picking/mainapp/carico_settori.ui");

    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), MainAppCaricoSettori, frm_settori );
...
MainAppCaricoSettori * main_app_carico_settori_new (MainAppWindow *win)
{
    ...
    MainAppCaricoSettori *w = g_object_new (MAIN_APP_CARICO_SETTORI_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);

    MainAppCaricoSettoriPrivate *priv = main_app_carico_settori_get_instance_private (MAIN_APP_CARICO_SETTORI (w));
