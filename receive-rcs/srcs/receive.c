...
ep_bool_t CheckDatiRicezione( void )
{
	...
	if(FileExists(szImportMonitor)){
		...
		DBRes=DBExecQuery(Cfg.nDebugLevel,"truncate import_ric_ord;"); DBclear(DBRes);
		DBRes=DBExecQuery(Cfg.nDebugLevel,"truncate import_ric_art;"); DBclear(DBRes);
			...
			*/
			if(FileExists(szImportFile)){
				...
				trace_debug(TRUE, TRUE, "DATI : Rinomino %s in %s.%s-%s%s",
					...
					GetTimeHHMMSS(szTimeBuffer),bOK?"":"-KO");
#endif
	...
	*/
	if(nUpdated[0]){
		...
		// Importo in ric_art
		DBResOrdini=DBExecQuery(Cfg.nDebugLevel>1,"insert into ric_art (RANMCED,ORDPROG,RACDPRO,RAQTORD) select  RANMCED, ORDPROG, RACDPRO, RAQTORD from import_ric_art where raflric='%c'",IMPORT_ARTICOLO_OK);
		...
		// Importo in catalogo
		DBResOrdini=DBExecQuery(Cfg.nDebugLevel>1,"insert into catalogo ( PRNMCED, PRCDPRO, PRTPART, PRDSTIT, PRPESGR, PRLUNGH, PRLARGH, PRALTEZ) select  PRNMCED, PRCDPRO, PRTPART, PRDSTIT, PRPESGR, PRLUNGH, PRLARGH, PRALTEZ from import_catalogo where prflric='%c'",IMPORT_CATALOGO_OK);
		sprintf(szMsg,"TABELLA ARTICOLI: importati %d articoli",atoi(DBcmdTuples(DBResOrdini)));
		if(bAlone==FALSE){
			SendMessage(Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
		} else {
		...
		if(nTuples){
			...
			for(nIndex=0;nIndex<nTuples;nIndex++){
				...
				*/
				...
				DBResUpdate=DBExecQuery(Cfg.nDebugLevel>1, "update ric_ord set ronmcpe=%d,ronmrgh=%d,ropspre=%d,rostato='%c',rotmrcz='now',ordtipo='%s',ubitipo='%s' where ordprog='%s';", 
					nCopie, nRighe, nPeso,
