...
* Data       Descrizione
* ---------- ---------------------------------------------------------------------
* 02-12-2017 Utiulizzo di receive-rcs per ricezione dati su impianto mondadori rcs
* ---------- ---------------------------------------------------------------------
*/
#include <glib.h>
...
int main(int argc,char **argv)
{
	...
	*/
	...
	trace_out_vstr(1,"Global Version  : %s",__version__);
	trace_out_vstr(1,"Module Version  : %s",__module_version__);
	...
	trace_out_vstr(1,"Configuration   : %s",__configuration_file__);
...
#ifdef TRACE
			trace_out_vstr_date(1, "Cannot open main messages queue");
#endif
		}
	}
	...
	*/
...
#ifdef TRACE
		trace_out_vstr(1,"Connection to database [%s:%s] failed.", Cfg.szDBHost,Cfg.szDBName);
#endif
		SafeIntFunc();
		exit(1);
	}
	...
	if(bAlone==FALSE){
		...
		*/
		SendMessage(ProcList,Cfg.nMainID, nPID,  PROGRAM_STARTED, NULL);
	}
	...
	for(;;){
		if(!CheckDatiRicezione()){
			if(bAlone==FALSE){
				...
				SendMessage(ProcList,Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
			} else {
#ifdef TRACE
...
BOOL GetTipoUbicazione(char *szOrdProg,char *szTipoOrdine,char *szTipoUbicazioni)
{
	...
	DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select tipoubi from tipo_ubi where ordtipo='%s';",szTipoOrdine);
	/* verifica risultato */
...
#ifdef TRACE
		trace_out_vstr_date(1,"ATTENZIONE ! : Ordine [%s - %s] Errore in attribuzione tipo ubicazione", szOrdProg,szTipoOrdine); 
#endif
	}
	DBclear(DBRes);
...
BOOL CheckDatiRicezione( void )
{
	...
	if(FileExists(szImportMonitor)){
		...
		DBRes=DBExecQuery(Cfg.nDebugVersion,"truncate import_ric_ord;"); DBclear(DBRes);
		DBRes=DBExecQuery(Cfg.nDebugVersion,"truncate import_ric_art;"); DBclear(DBRes);

		DBRes=DBExecQuery(Cfg.nDebugVersion,"BEGIN WORK;"); DBclear(DBRes);
				...
				*/
				if(bDelete[nIndex]){
					...
					if(bRecordPresente){
						...
						*/
						DBRes=DBExecQuery(Cfg.nDebugVersion>1,"drop table %s_copy;",db[nIndex].szTableName); 
						DBclear(DBRes);
						/*
						* Creo la copia della tabelle e inserisco il contenuto
						*/
						DBRes=DBExecQuery(Cfg.nDebugVersion>1,"create table %s_copy as select * from %s;",
							db[nIndex].szTableName,db[nIndex].szTableName); DBclear(DBRes);

						/*
						* cancello il contenuto della tabella 
						*/
						DBRes=DBExecQuery(Cfg.nDebugVersion,"delete from %s;",db[nIndex].szTableName); DBclear(DBRes);
					}
				}
				...
				if(bAlone==FALSE){
					...
					*/
					...
					if(bOK){
						...
						trace_out_vstr_date(1, "Errore in ricezione [%s]",db[nIndex].szTableName);
					}
#endif
				}
			} else {
...
#ifdef TRACE
				trace_out_vstr_date(1,"DATI : Rinomino %s in %s.%s-%s%s",
					szImportFile,
					szImportFile,
		...
		if(FileExists(szImportMonitor)){
#ifdef TRACE
			trace_out_vstr_date(1,"MONITOR : Rinomino %s in %s.%s-%s%s",
				szImportMonitor,
				szImportMonitor,
				GetDateYYYYMMDD(time((long *)0),szDateBuffer),
				GetTimeHHMMSS(time((long *)0),szTimeBuffer),bOK?"":"-KO");
		...
		if(bOK){
			DBRes=DBExecQuery(Cfg.nDebugVersion,"commit work;"); DBclear(DBRes);
		} else {
			DBRes=DBExecQuery(Cfg.nDebugVersion,"abort work;"); DBclear(DBRes);
		}
	...
	*/
	if(nUpdated[0]){
#ifdef TRACE
		trace_out_vstr_date(1,"INIZIO POST RICEZIONE ORDINI");
#endif
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_ord set roflric='%c';",IMPORT_ORDINE_OK);
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_ord set roflric='%c' where ordprog in (SELECT ordprog from ric_ord);",IMPORT_ORDINE_DOPPIO);
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_ord set roflric='%c' where ordprog not in (SELECT distinct ordprog from import_ric_art);",IMPORT_ORDINE_SENZA_ARTICOLI);
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_art set raflric='%c';",IMPORT_ARTICOLO_OK);
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_art set raflric='%c' where ordprog||racdpro in (SELECT ordprog||racdpro from ric_art);",IMPORT_ARTICOLO_DOPPIO);
		...
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_art set raflric='%c' where ordprog not in (SELECT distinct ordprog from import_ric_ord);",IMPORT_ARTICOLO_SENZA_ORDINE);
		...
		DBRes=DBExecQuery(Cfg.nDebugVersion>1,"update import_ric_art set raflric='%c' where ordprog in (SELECT distinct ordprog from import_ric_ord where roflric='%c');",IMPORT_ARTICOLO_CON_ORDINE_DOPPIO,IMPORT_ORDINE_DOPPIO);
		...
		DBRes=DBExecQuery(Cfg.nDebugVersion>1,"update import_catalogo set prflric='%c';",IMPORT_CATALOGO_OK);
		...
		// Importo in ric_ord
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"insert into ric_ord ( RONMCED, ORDPROG, ROCDLIN, RODSLIN, ROCDRID, RODSCLI, ROINCLI, ROLOCLI, ROCPCLI, ROPRCLI, ROFLPAR, ROFLARR, RODSARR, RODTBCD, ROTPBCD, RODSNOT, ROSWCOL, ROSGBNC ) select RONMCED, ORDPROG, ROCDLIN, RODSLIN, ROCDCLI, RODSCLI1, ROINCLI, ROLOCLI, ROCPCLI, ROPRCLI, ROFLPAR, ROFLARR, RODSARR, RODTBCD, ROTPBCD, RODSNOT, ROSWPLT, ROSGBNC from import_ric_ord where roflric='%c'",IMPORT_ORDINE_OK);

		sprintf(szMsg,"TABELLA ORDINI: importati %d ordini",atoi(DBcmdTuples(DBResOrdini)));
		if(bAlone==FALSE){
			SendMessage(ProcList,Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
		} else {
			fprintf(stderr,szMsg);
		...
		// Importo in ric_art
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"insert into ric_art (RANMCED,ORDPROG,RACDPRO,RAQTORD) select  RANMCED, ORDPROG, RACDPRO, RAQTORD from import_ric_art where raflric='%c'",IMPORT_ARTICOLO_OK);

		sprintf(szMsg,"TABELLA ARTICOLI: importati %d articoli",atoi(DBcmdTuples(DBResOrdini)));
		if(bAlone==FALSE){
			SendMessage(ProcList,Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
		} else {
			fprintf(stderr,szMsg);
		...
		// Importo in catalogo
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"insert into catalogo ( PRNMCED, PRCDPRO, PRTPART, PRDSTIT, PRPESGR, PRLUNGH, PRLARGH, PRALTEZ) select  PRNMCED, PRCDPRO, PRTPART, PRDSTIT, PRPESGR, PRLUNGH, PRLARGH, PRALTEZ from import_catalogo where prflric='%c'",IMPORT_CATALOGO_OK);
		sprintf(szMsg,"TABELLA ARTICOLI: importati %d articoli",atoi(DBcmdTuples(DBResOrdini)));
		if(bAlone==FALSE){
			SendMessage(ProcList,Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
		} else {
		...
		*/
		DBResOrdini=DBExecQuery(Cfg.nDebugVersion>1,"select ordprog from ric_ord where rostato IS NULL;");
		...
		if(nTuples){
			...
			for(nIndex=0;nIndex<nTuples;nIndex++){
				...
				DBResRighe=DBExecQuery(Cfg.nDebugVersion>1,"select racdpro,raqtord from ric_art where ordprog='%s';",szOrdProg);
				...
				for(nRigaIndex=0;nRigaIndex<nRighe;nRigaIndex++){
					...
					DBResCat=DBExecQuery(Cfg.nDebugVersion>1,"select prpesgr from catalogo where prcdpro='%s';",szRACDPRO);
					if(DBntuples(DBResCat)){
						nPeso+=atoi(DBgetvalue(DBResCat,0,0))*nCopie;
					} else {
						bOK=FALSE;
#ifdef TRACE
						trace_out_vstr_date(1,"Prodotto [%s] non presente in catalogo",szRACDPRO);
#endif
				...
				*/
				/*
				GetTipoUbicazione(szOrdProg,szTipoOrdini,szTipoUbicazioni);
				*/
				...
				*/
				...
				DBResUpdate=DBExecQuery(Cfg.nDebugVersion>1, "update ric_ord set ronmcpe=%d,ronmrgh=%d,ropspre=%d,rostato='%c',rotmrcz='now',ordtipo='%s',ubitipo='%s' where ordprog='%s';", 
					nCopie, nRighe, nPeso,
		...
		DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select ronmced ,count(ronmced) ,sum(ronmrgh) ,sum(ronmcpe) from ric_ord where ubitipo='%s' and rostato='%c' group by ronmced order by ronmced;",Cfg.szTipoUbicazioni,ORDINE_ATTESA_RICEZIONE);
		if(DBntuples(DBRes)){
		...
		if(bOK){
			DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select cdnmced from cedole where cdnmced=%d",nNMCED);
			...
			if(bOK){
				DBRes=DBExecQuery(Cfg.nDebugVersion>1,"insert into cedole (cdnmced,cdnmord,cdnmrgh,cdnmcpe,cdstato,cdtmulm) values (%d,%d,%d,%d,'%c','now');", nNMCED, nNMORD, nNMRGH, nNMCPE,CEDOLA_RICEVUTA);
				DBclear(DBRes);
			}
		}
		DBResUpdate=DBExecQuery(Cfg.nDebugVersion>1, "update ric_ord set rostato='%c' where rostato='%c';", 
			ORDINE_RICEVUTO,ORDINE_ATTESA_RICEZIONE);
		DBclear(DBResUpdate);


#ifdef TRACE
		trace_out_vstr_date(1,"FINE POST RICEZIONE ORDINI");
#endif
	...
	if(nUpdated[2]){
		/* Catalogo */
		sprintf(szBuffer,"[%d] Titoli ",nUpdated[2]);
		strcat(szMsgData,szBuffer);
	}
	...
	if(bRicezione){
		if(strlen(szMsgData)){
			...
			*/
			if(bAlone==FALSE){
				SendMessage(ProcList,Cfg.nMainID, nPID,  DISPLAY_MSG, szMsg);
				SendMessage(ProcList,Cfg.nMainID, nPID,  REFRESH_DATA, szMsg);
			}
#ifdef TRACE
	...
	for(nIndex=0; nIndex<nTables; nIndex++){
		...
		*/
		...
		if(bOK){
			...
			DBRes=DBExecQuery(Cfg.nDebugVersion>1,"vacuum analyze %s;",db[nIndex].szTableName);
			DBclear(DBRes);
		}
	}
...
#ifdef TRACE
	trace_out_vstr_date(1,"Read Configuration : file %s not exists",szCfgFileName);
#endif
		return FALSE;
	}
...
void DeleteBackupFiles(void)
{
	...
	sprintf(szCommand,"rm -f %s/\?\?\?\?%02d\?\?-*",
					...
					nMesePrecedente);
#ifdef TRACE
	trace_out_vstr(1,"DeleteBackupFiles() : Eseguo Comando : [%s]",szCommand);
#endif
	system(szCommand);
}
...
void do_elaborazione_kit(char *szCDKIT,int nNMCED)
{
	...
	DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select ktcdpro from kit where ktcdpro='%s' and ktnmced=%d;",szCDKIT,nNMCED);
	if (DBresultStatus(DBRes) != DBRES_TUPLES_OK || DBntuples(DBRes)==0){
#ifdef TRACE
		trace_out_vstr(1,"KIT : Errore in ricerca codice kit [%s]",szCDKIT);
#endif
		bOK=FALSE;
	}
	DBclear(DBRes);
	...
	DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select pkcdpro,pknmcpe from prodotti_kit where pkcdkit='%s';",szCDKIT);
	...
	*/
	DBRes=DBExecQuery(Cfg.nDebugVersion>1,"begin work;");
	DBclear(DBRes);
	

	DBResOrd=DBExecQuery(Cfg.nDebugVersion>1,"select ordprog,ronmrgh,ronmcpe from ric_ord where ronmced='%d' order by ordprog;",nNMCED);
	...
	for(nIndex=0;nIndex<nOrdini;nIndex++){
		...
		*/
		...
		DBResArt=DBExecQuery(Cfg.nDebugVersion>1,"select racdpro,raqtord from ric_art where ordprog='%s' and racdpro in (%s);",szOrdprog,szSetProdottiKit);
		...
		if(nArticoli==nProdottiKit){
			...
			for(nArticoliIndex=0;nArticoliIndex<nArticoli;nArticoliIndex++){
				...
				DBRes=DBExecQuery(Cfg.nDebugVersion>1,"select pkcdpro,pknmcpe from prodotti_kit where pkcdkit='%s' and pkcdpro='%s';",
					szCDKIT,szCDPRO);
		...
		*/
		if(bOrdineOK){
			...
			DBResInsert=DBExecQuery(Cfg.nDebugVersion>1,"insert into ric_art (ranmced,ordprog,racdpro,raqtord) values ('%d','%s','%s',%d);",
			...
			for(nArticoliIndex=0;nArticoliIndex<nArticoli;nArticoliIndex++){
				...
				if(nCopieProdotto==nNumeroKit*nCopieKit){
					...
					*/
					DBResDelete=DBExecQuery(Cfg.nDebugVersion>1,"delete from ric_art where ordprog='%s' and racdpro='%s';", szOrdprog,szCDPRO);
					DBclear(DBResDelete);
					/* ricalcolo i valori di righe e copie per ordine */
					nCopieOrdine-=nCopieKit*nNumeroKit;
					nRigheOrdine-=1;
				} else {
					DBResUpdate=DBExecQuery(Cfg.nDebugVersion>1,"update ric_art set raqtord=raqtord-%d where ordprog='%s' and racdpro='%s';",
			...
			DBResUpdate=DBExecQuery(Cfg.nDebugVersion>1,"update ric_ord set ronmcpe=%d,ronmrgh=%d where ordprog='%s';",nCopieOrdine,nRigheOrdine,szOrdprog);
			DBclear(DBResUpdate);
