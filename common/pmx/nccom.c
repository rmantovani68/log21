                ...
                if (end_char == buf[i] &&
                    ...
                    if (msg->do_checksum) {
                        ...
                        if (c != checksum) {
                            ...
                            *num_msg = 1;
#ifdef TRACE
                            trace_out_vstr(0," [checksum error 0x%x]", checksum);
                            endtime();
#endif    /* TRACE */
...
int RecSerMsg(int fd, char *buf, MSG_SPEC *mess, int *msg_list, int *num_msg, int *msg_len, int to_before, int to_within)
{
    return 0;
}
