...
echo "DROP FUNCTION IF EXISTS  ubicazione_rid(bpchar);"     | psql -U $PGUSER -h $PGHOST $PGDB >> crea_tabelle.out 2>&1 ;echo "CREATE FUNCTION ubicazione_rid(bpchar) returns text as 'select substr(\$1,1,1)||''.''||substr(\$1,8,2)||''.''||substr(\$1,12,1)' language 'sql';" | psql -U $PGUSER -h $PGHOST $PGDB >> crea_tabelle.out 2>&1 ; echo -n .
