...
*/
drop function if exists fn_diff_sped(text);
	...
	FOR ordini IN EXECUTE query_text LOOP
		...
		FOR articoli IN SELECT * FROM ric_art where ordprog=ordini.ordprog order by racdpro LOOP
			...
			IF qtspe = NULL THEN
				...
				insert into diff_sped values (ordini.rodtbam,cedola,ordini.ordprog,ordini.rocdrid,ordini.rodscli,ordini.ronmrgh,articoli.racdpro,qtord,0,qtord);
				nrecords := nrecords + 1;
			END IF;
			IF qtord != qtspe THEN
				diff := qtord-qtspe;
