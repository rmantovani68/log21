...
SMTP_ERROR=$(smtp-cli --host $Mail_Host --auth-plain --user=$Mail_User --pass=$Mail_Password --from $Mail_User \
$Many_Mail_To $Cc_String --subject "$Mail_Subject" $Many_Mail_Attach "$Body_String" --missing-modules-ok 2>&1)

#Se la stringa di errore non e' vuota allora e' successo qualcosa
if [ ! -n "$SMTP_ERROR" ]
then
	echo "Spedita mail"
	echo "insert into mail_sender_log (message,code) values ('Spedita Mail a $Many_Mail_To', 0)"|psql -h database mr
	exit 0
else
	echo "Mail non spedita"
	## tolgo eventuali apici che mi fanno casino nella query
	echo "insert into mail_sender_log (message,code) values ('Errore in spedizione a $Many_Mail_To: $(echo $SMTP_ERROR|tr -d \"\'\")',1)"|psql -h database mr
	exit 1
fi
