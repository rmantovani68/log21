...
BOOL DatiVettoreDC(PDATIORDINE pDatiOrdine,char *szCDVET,int nPos1,int nPos2,int nPos3)
{
	...
	PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select elcdsvet,elcinvet,elclovet from ttve where elccdsoc='%s' and elccdmag='%s' and elccdvet='%s';",
		...
		szCDVET);
	if(PQntuples(PGRes)){
		InitData("EveryPage",DBgetvalue(PGRes,0,0),DCPos[nPos1].nSR,DCPos[nPos1].nSC,FALSE);
		InitData("EveryPage",DBgetvalue(PGRes,0,1),DCPos[nPos2].nSR,DCPos[nPos2].nSC,FALSE);
		InitData("EveryPage",DBgetvalue(PGRes,0,2),DCPos[nPos3].nSR,DCPos[nPos3].nSC,FALSE);
	} else {
#ifdef TRACE
...
/*
* StampaDC(PDATIORDINE pDatiOrdine,BOOL bStorico)
*
...
#ifdef TRACE
	trace_out_vstr_date(1,"Stampa DC [%s]",pDatiOrdine->szORDPROG);
#endif
	/* 
	* Decido se usare EURO o LIRE 
	*/
...
//	DBclear(PGRes);
	...
	*/
	if(bOK && !bStorico){
		PGRes=DBExecQuery(Cfg.nDebugVersion>2,"update ric_ord set rostato='%c' where ordprog='%s';",
			ORDINE_STAMPATA_XAB,
			pDatiOrdine->szORDPROG);
...
#ifdef TRACE
	trace_out_vstr_date(1,"Stampa DC Spedizione [%s] Storico : %d",szOrdProg,bStorico);
#endif
...
#ifdef TRACE
	trace_out_vstr_date(1,"Stampa DC Linea [%s - %s - %s - %s]",szNMCED,szTPSPE,szCDLIN, szCDVET);
#endif
	sprintf(szPrintFile,"%s/%s.dc",Cfg.szPathStampe,pszPrinterName);
