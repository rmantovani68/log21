...
BOOL bEnableNewPage=TRUE;		/* Stampa dati su nuova pagina abilitata/disabilitata 										*/
BOOL bEnableOldPage=TRUE;		/* Stampa dati al cambio pagina su vecchia pagina abilitata/disabilitata 	*/
BOOL bEnableEveryPage=TRUE;	/* Stampa dati al cambio pagina su nuova pagina abilitata/disabilitata 		*/
BOOL bStato=FALSE;	/* E' TRUE se si e' riuscito ad aprire in scrittura il file di anteprima di stampa
...
int InitData(char *pszDataType,char *pszString,int nRow,int nColumn,BOOL bIncrement)
{
	int nTipoDato=0;
	int nRetVal=0;
...
BOOL PrintField(char *pszString,int nRow,int nColumn)
{
	int nSize;
	char *pszApp;
	BOOL bRetVal=FALSE;
...
int PrintRow(char *pszRow,int nSkip,BOOL bA4)
{
	int nRetVal=-1;
	int nSize;
...
BOOL SkipPage(BOOL bPrintOldPage,BOOL bResetPageNumber,BOOL bA4)
{
	BOOL bRetVal=FALSE;
...
*/
void EnableNewPage(BOOL Enable)
{
...
* La funzione non ritorna valori.
*/
void EnableOldPage(BOOL Enable)
{
...
* La funzione non ritorna valori.
*/
void EnableEveryPage(BOOL Enable)
{
...
BOOL DeleteData(char *pszDataType)
{
	int nTipoDato=0;
	BOOL bRetVal=TRUE;
...
void FillData(LINKED_LIST *pList,char *pszString,int nRow,int nColumn,BOOL bIncrement)
{
	int nSize;
	PDATA pElem;
...
*/
BOOL SearchString(char *pszString1,char *pszString2,char *pszString3)
{
	int nLen1,nLen2;
	int nIndex=1;
	char *pszChar;
...
*/
void PutChangedPageInFile(BOOL bResetPageNumber,BOOL bA4)
{
	if (nPag==1){
		PrintList(&FirstPageList);
		PrintList(&EveryPageList);
...
*/
void PutPageInFile(BOOL bA4,BOOL bFormFeed)
{
	int nIndex;
...
BOOL VerifyAll(char *pszFileName)
{
	int nRetClose;
	BOOL bRetVal=TRUE;
	FILE *fpCheck;
...
void DataVerify(FILE *fpCheck,LINKED_LIST *pList)
{
	...
	pData=link_first(pList);
	while (pData){
		fprintf(fpCheck,"Puntatore: %x\n",(unsigned int)pData);
		fprintf(fpCheck,"Precedente: %x\n",(unsigned int)pData->link.p);
		fprintf(fpCheck,"Successivo: %x\n",(unsigned int)pData->link.n);
