...
BOOL DatiVettore_A4(PDATIORDINE pDatiOrdine,char *szCDVET,int nPos1,int nPos2,int nPos3)
{
	...
	PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select elcdsvet,elcinvet,elclovet from ttve where elccdsoc='%s' and elccdmag='%s' and elccdvet='%s';",
		...
		szCDVET);
	if(DBntuples(PGRes)){
		InitData("LastPage",DBgetvalue(PGRes,0,0),XABPos_A4[nPos1].nSR,XABPos_A4[nPos1].nSC,FALSE);
		InitData("LastPage",DBgetvalue(PGRes,0,1),XABPos_A4[nPos2].nSR,XABPos_A4[nPos2].nSC,FALSE);
		InitData("LastPage",DBgetvalue(PGRes,0,2),XABPos_A4[nPos3].nSR,XABPos_A4[nPos3].nSC,FALSE);
	} else {
#ifdef TRACE
...
{
	DBresult *PGRes=NULL;
	DBresult *PGResTot=NULL;
	DBresult *PGResCat=NULL;
	DBresult *PGResRA=NULL;
	DBresult *PGResCollo=NULL;
	BOOL bOK=TRUE;
	char szTmpBuf[80];
	char szBuffer[128];
	...
	*/
	...
	switch(Cfg.nTipoOrdinamentoStampaXAB){
		case 1:
		{
			PGResTot=DBExecQuery(Cfg.nDebugVersion>2,"select rpcdpro,count(rpcdpro),sum(rpqtspe) from %s where ordprog='%s' group by rpcdpro order by rpcdpro;",szRigProd,pDatiOrdine->szORDPROG);
			if((nTotTuples=DBntuples(PGResTot))){
				for(nTotIndex=0;nTotIndex<nTotTuples;nTotIndex++){
					strcpy(szRPCDPRO,DBgetvalue(PGResTot,nTotIndex,0));
					nRigheProdotto=atoi(DBgetvalue(PGResTot,nTotIndex,1));
					nRPQTSPETot=atoi(DBgetvalue(PGResTot,nTotIndex,2));


					PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select rpqtspe,rpnmcol from %s where ordprog='%s' and rpcdpro='%s' order by rpnmcol;",szRigProd,pDatiOrdine->szORDPROG,szRPCDPRO);
					if((nTuples=DBntuples(PGRes))){
						for(nIndex=0;nIndex<nTuples;nIndex++){
							nRPQTSPE=atoi(DBgetvalue(PGRes,nIndex,0));
							nRPNMCOL=atoi(DBgetvalue(PGRes,nIndex,1));
							...
							/*
							* 02-08-2005 LG : tolta la quantita' in lettere
							*/
					...
					if (nRigheProdotto>1){
						sprintf(szBuffer,"                                                      TOT: %5d", nRPQTSPETot); strcpy(szRiga,szBuffer);
						PrintRow(szRiga,1,TRUE);
					} 
		...
		case 2:
		{
			PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select rpcdpro,rpqtspe,rpnmcol from %s where ordprog='%s' order by rpcdpro,rpnmcol;",szRigProd,pDatiOrdine->szORDPROG);
			if((nTuples=DBntuples(PGRes))){
				for(nIndex=0;nIndex<nTuples;nIndex++){
					strcpy(szRPCDPRO,DBgetvalue(PGRes,nIndex,0));
					nRPQTSPE=atoi(DBgetvalue(PGRes,nIndex,1));
					nRPNMCOL=atoi(DBgetvalue(PGRes,nIndex,2));
		...
		case 3:
		{
			...
			PGResCollo=DBExecQuery(Cfg.nDebugVersion>2,"select cpnmcol from %s where ordprog='%s' order by cpnmcol;",szColProd,pDatiOrdine->szORDPROG);

			if((nColli=DBntuples(PGResCollo))){

				for(nColloIndex=0;nColloIndex<nColli;nColloIndex++){


					PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select rpcdpro,rpqtspe from %s where ordprog='%s' and rpnmcol=%d order by rpprrig;",szRigProd,pDatiOrdine->szORDPROG,nColloIndex+1);
					if((nTuples=DBntuples(PGRes))){
						for(nIndex=0;nIndex<nTuples;nIndex++){
							strcpy(szRPCDPRO,DBgetvalue(PGRes,nIndex,0));
							nRPQTSPE=atoi(DBgetvalue(PGRes,nIndex,1));
	...
	*/
	if(bOK && !bRistampa){
		PGRes=DBExecQuery(Cfg.nDebugVersion>2,"update %s set roprxab=%d,roaaxab=%d,rodtxab='%s' where ordprog='%s';",
			szRicOrd,
			pDatiOrdine->nROPRXAB,
			pDatiOrdine->nROAAXAB,
			pDatiOrdine->szRODTXAB,
			pDatiOrdine->szORDPROG);
		if((DBresultStatus(PGRes)!=DBRES_COMMAND_OK)){
			bOK=FALSE;
		}
		DBclear(PGRes);
	}
...
#ifdef TRACE
	trace_out_vstr_date(1,"Stampa Packing List Linea [%s - %s - %s]",szNMCED,szTPSPE,szCDLIN);
#endif
	sprintf(szPrintFile,"%s/%s.PL_Linea_A4",Cfg.szPathStampe,pszPrinterName);

	PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select ordprog from %s where ordtipo='%s' and ronmced='%s' and rotpspe='%s' and rocdlin='%s' order by ordprog;",szRicOrd,Cfg.szTipoOrdini,szNMCED,szTPSPE,szCDLIN);
	if((nTuples=DBntuples(PGRes))){
		/* Creazione del file di stampa */
		InitPrint(szPrintFile,MAX_RIGHE_XAB_A4,MAX_COLONNE_XAB_A4,Cfg.nPrimaRigaXAB_A4,Cfg.nUltimaRigaXAB_A4);
