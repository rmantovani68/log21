...
BOOL StampaBolla(FILE *fp, char *szOrdProg)
{
...
#ifdef TRACE
	trace_out_vstr_date(1,"StampaBolla(%s)",szOrdProg);
#endif
	...
	if(GetDatiOrdine(szOrdProg,&DatiOrdine,FALSE)){
		...
		for(nColloIndex=0;nColloIndex<DatiOrdine.nRONMCLL;nColloIndex++){
			if(GetDatiCollo(szOrdProg,nColloIndex+1,&DatiCollo,FALSE)){
				...
				PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select rp.rpcdpro,rp.rpcdubi,rp.rpqtspe,ra.rapzpre,ca.prdstit,ca.prdsaut \
				...
				if((nTuples=DBntuples(PGRes))){
					...
					for(nRigaIndex=0;nRigaIndex<nTuples;nRigaIndex++){
						strcpy(DatiRiga.szRPCDPRO,DBgetvalue(PGRes,nRigaIndex,0));
						strcpy(DatiRiga.szRPCDUBI,DBgetvalue(PGRes,nRigaIndex,1));
						DatiRiga.nRPQTSPE=atoi(DBgetvalue(PGRes,nRigaIndex,2));
						DatiRiga.nRAPZPRO=atoi(DBgetvalue(PGRes,nRigaIndex,3));
						strcpy(DatiRiga.szPRDSTIT,DBgetvalue(PGRes,nRigaIndex,4));
						strcpy(DatiRiga.szPRDSAUT,DBgetvalue(PGRes,nRigaIndex,5));
	...
	if (bOK) {
		...
		*/
		...
		PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select RNFRTXT from ric_note where ordprog='%s' and rntptxt='%s' order by rnnmtxt;",
		...
		if((nTuples=DBntuples(PGRes))){
			...
			*/
			for(nIndex=0;nIndex<nTuples;nIndex++){
				...
				PrintRC(nPrintingRow[nRigheStampate], Pos[F_NOTE].nSC, DBgetvalue(PGRes,nIndex,0));
				nRigheStampate++;
				if (nRigheStampate==Cfg.nPrintableRow) {
					/*
					* Nuova pagina
...
#ifdef TRACE
	trace_out_vstr_date(1,"PrintBollaRAC(%s)",szOrdProg);
#endif
...
BOOL PrintRACLinea(char *pszPrinterName,char *szNMCED,char *szTPSPE,char *szCDLIN) 
{
...
#ifdef TRACE
	trace_out_vstr_date(1,"PrintRACLinea(%s-%s:%s:%s)",pszPrinterName,szNMCED,szTPSPE,szCDLIN);
#endif
	...
	*/
	...
	if ((fp=fopen(szFileName,"w"))!=(FILE *)NULL) {
		...
		PGRes=DBExecQuery(Cfg.nDebugVersion>2,"select ordprog from ric_ord where rostato='%c' and ordtipo='%s' and ronmced='%s' and rotpspe='%s' and rocdlin='%s' order by roprgln;", ORDINE_ELABORATO,Cfg.szTipoOrdini,szNMCED,szTPSPE,szCDLIN);
		if((nTuples=DBntuples(PGRes))){
			for(nIndex=0;nIndex<nTuples;nIndex++){
				if(StampaBolla(fp, DBgetvalue(PGRes,nIndex,0))==FALSE){
					sprintf(szMsg,"Errore in stampa RAC ordine [%s]\n",DBgetvalue(PGRes,nIndex,0));
					SendMessage(Cfg.nMainID, nPID, DISPLAY_MSG, szMsg);
#ifdef TRACE
					trace_out_vstr_date(1,"Errore in stampa RAC ordine [%s]",DBgetvalue(PGRes,nIndex,0));
#endif
				} else {
					SendMessage(Cfg.nMainID, nPID, PRINTLABEL_RAC_STAMPATO,DBgetvalue(PGRes,nIndex,0));
				}
			}
		} else {
			bOK=FALSE;
...
#ifdef TRACE
		trace_out_vstr_date(1,"Errore in apertura file [%s]",szFileName);
#endif
		bOK=FALSE;
	}
	if (bOK){
...
#ifdef TRACE
	trace_out_vstr_date(1,"PrintBolleRAC(%s)",pszPrinterName);
#endif
