# ---------------------------------------------------------
# common.mak : include file con definizioni generali da
...
ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK4)","")
	HAS_GTK4 != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK4
else 
	HAS_GTK4 = 1
endif

ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK)","")
	HAS_GTK != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK
else 
	ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK4)","")
		HAS_GTK4 != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GTK4
		HAS_GTK = 0
...
ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_NCURSES)","")
	HAS_NCURSES != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_NCURSES
else 
	HAS_NCURSES = 1
endif

ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_POSTGRES)","")
	HAS_POSTGRES != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_POSTGRES
else 
	HAS_POSTGRES = 1
endif

ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GW)","")
	HAS_GW != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_GW
else 
	HAS_GW = 0
endif

ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_ODBC)","")
	HAS_ODBC != cat $(BASE_DIRECTORY)/$(TARGET_NAME)/config/HAS_ODBC
else 
	HAS_ODBC = 0
endif
...
$(info HAS_GTK4     : $(HAS_GTK4))
$(info HAS_NCURSES  : $(HAS_NCURSES))
$(info HAS_POSTGRES : $(HAS_POSTGRES))
$(info HAS_ODBC     : $(HAS_ODBC))

GTK4_LIBS   = `pkg-config gtk4 --libs`
GTK4_CFLAGS = `pkg-config gtk4 --cflags`
...
MSQ_LIB = -L$(BASE_DIRECTORY)/common-gtk4/pmx -lPmxMsq
MSQ_INC = -I$(BASE_DIRECTORY)/common-gtk4/pmx

# libreria GimpWidgets
GW_LIB = -L$(BASE_DIRECTORY)/common-gtk4/gw -lGW
GW_INC = -I$(BASE_DIRECTORY)/common-gtk4/gw

# libreria Easy Picking
EP_LIB = -L$(BASE_DIRECTORY)/common-gtk4/ep -lEP
EP_INC = -I$(BASE_DIRECTORY)/common-gtk4/ep
...
LFLAGS = $(EP_LIB) $(MSQ_LIB) $(ZMQ_LIBS) $(GLIB_LIBS)  $(MEMCACHED_LIBS) -lm 
CFLAGS += $(EP_INC) $(MSQ_INC) $(ZMQ_CFLAGS) $(GLIB_CFLAGS) $(MEMCACHED_CFLAGS)
...
ifneq ("$(wildcard $(BASE_DIRECTORY)/$(TARGET_NAME)VERSION)","")
	VERSION != cat VERSION
else
	VERSION =
endif
...
$(TARGET_EXECUTABLE): $(_OBJS_) $(_SRCS_OBJS_)
	...
	@rm -rf $(BASE_DIRECTORY)/exe/org.easy-picking.$(TARGET_NAME).gschema.valid

ifeq (1,${COMPILE_RESOURCES}) 

srcs/resources.c: $(BASE_DIRECTORY)/$(TARGET_NAME)/ui/$(TARGET_NAME).gresource.xml $(shell $(GLIB_COMPILE_RESOURCES) --sourcedir=$(BASE_DIRECTORY)/$(TARGET_NAME)/ui --generate-dependencies $(BASE_DIRECTORY)/$(TARGET_NAME)/ui/$(TARGET_NAME).gresource.xml)
	@echo 'R - $<'
	@$(GLIB_COMPILE_RESOURCES) $(BASE_DIRECTORY)/$(TARGET_NAME)/ui/$(TARGET_NAME).gresource.xml --target=$@ --sourcedir=$(BASE_DIRECTORY)/$(TARGET_NAME)/ui --generate-source
...
clean: 
	@rm -f $(_OBJS_) $(BASE_DIRECTORY)/$(TARGET_NAME)/srcs/cmp_time.c $(BASE_DIRECTORY)/$(TARGET_NAME)/includes/cmp_time.h $(_SRCS_OBJS_) $(TARGET_EXECUTABLE) $(BASE_DIRECTORY)/$(TARGET_NAME)/srcs/resources.c
