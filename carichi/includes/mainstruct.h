...
typedef struct tagCfgStruct{
    ...
    char szImportCarichi[128];
    char szExportRimanenze[128];
    char szExportFlusso[128];
    ...
    ep_bool_t bShowListaMovimentazioniInDlgEditProdotto;

    int nSpedizioniAutomaticheDelay;
    int nCheckRicezioneDatiDelay;

    int nDelay;
} CFGSTRUCT, *PCFGSTRUCT;

typedef struct tagGiacenzeDatiRicercaStruct{
    char szNumeroDoc[32];
    char szCodiceUdc[32];
    char szCodiceProdotto[32];
    char szTitoloProdotto[32];
    char szCausale[32];
    int nDistinta ;
    char szDataDal[128];
    char szDataAl[128];
} GIACENZE_DATI_RICERCA_STRUCT, *P_GIACENZE_DATI_RICERCA_STRUCT;
