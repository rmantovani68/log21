...
#include "mainapp.h"
#include "mainwin.h"
#include "ingresso_merce.h"
#include "mainfun.h"

#define WINDOW_NAME "ingresso-merce-window"

static GtkWindow *_parent;

struct _IngressoMerceWindow
{
    GtkDialog parent;
};

typedef struct _IngressoMerceWindowPrivate IngressoMerceWindowPrivate;

struct _IngressoMerceWindowPrivate
{
    GtkWindow *parent;
...
G_DEFINE_TYPE_WITH_PRIVATE(IngressoMerceWindow, ingresso_merce_window, GTK_TYPE_DIALOG)
...
static void ingresso_merce_window_init (IngressoMerceWindow *win)
{
    IngressoMerceWindowPrivate *priv = ingresso_merce_window_get_instance_private (win);
...
static void ingresso_merce_window_dispose (GObject *object)
{
    IngressoMerceWindow *win = INGRESSO_MERCE_WINDOW (object);
    IngressoMerceWindowPrivate *priv = ingresso_merce_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (ingresso_merce_window_parent_class)->dispose (object);
}

static void ingresso_merce_window_finalize (GObject *object)
{
    IngressoMerceWindow *win = INGRESSO_MERCE_WINDOW (object);
    IngressoMerceWindowPrivate *priv = ingresso_merce_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (ingresso_merce_window_parent_class)->finalize (object);
}

static void ingresso_merce_window_class_init (IngressoMerceWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = ingresso_merce_window_dispose;
    G_OBJECT_CLASS (class)->finalize = ingresso_merce_window_finalize;
...
IngressoMerceWindow * ingresso_merce_window_new (GtkWindow *parent)
{
    ...
    IngressoMerceWindow *win = g_object_new (INGRESSO_MERCE_WINDOW_TYPE, "transient-for", parent, "use-header-bar", TRUE, NULL);
    IngressoMerceWindowPrivate *priv = ingresso_merce_window_get_instance_private (win);
