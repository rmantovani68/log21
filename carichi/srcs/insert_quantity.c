...
#include "mainapp.h"
#include "mainwin.h"
#include "insert_quantity.h"
#include "mainfun.h"

#define WINDOW_NAME "insert-quantity-window"

static GtkWindow *_parent;

struct _InsertQuantityWindow
{
    GtkDialog parent;
};

typedef struct _InsertQuantityWindowPrivate InsertQuantityWindowPrivate;

struct _InsertQuantityWindowPrivate
{
    GtkWindow *parent;
...
G_DEFINE_TYPE_WITH_PRIVATE(InsertQuantityWindow, insert_quantity_window, GTK_TYPE_DIALOG)
...
static void insert_quantity_window_init (InsertQuantityWindow *win)
{
    InsertQuantityWindowPrivate *priv = insert_quantity_window_get_instance_private (win);
...
static void insert_quantity_window_dispose (GObject *object)
{
    InsertQuantityWindow *win = INSERT_QUANTITY_WINDOW (object);
    InsertQuantityWindowPrivate *priv = insert_quantity_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (insert_quantity_window_parent_class)->dispose (object);
}

static void insert_quantity_window_finalize (GObject *object)
{
    InsertQuantityWindow *win = INSERT_QUANTITY_WINDOW (object);
    InsertQuantityWindowPrivate *priv = insert_quantity_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (insert_quantity_window_parent_class)->finalize (object);
}

static void insert_quantity_window_class_init (InsertQuantityWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = insert_quantity_window_dispose;
    G_OBJECT_CLASS (class)->finalize = insert_quantity_window_finalize;
...
InsertQuantityWindow * insert_quantity_window_new (GtkWindow *parent)
{
    ...
    InsertQuantityWindow *win = g_object_new (INSERT_QUANTITY_WINDOW_TYPE, "transient-for", parent, "use-header-bar", TRUE, NULL);
    InsertQuantityWindowPrivate *priv = insert_quantity_window_get_instance_private (win);
