...
#define WINDOW_NAME "import-spedizioni-window"

static GtkWindow *_parent;

struct _ImportSpedizioniWindow
{
    GtkDialog parent;
};

typedef struct _ImportSpedizioniWindowPrivate ImportSpedizioniWindowPrivate;

struct _ImportSpedizioniWindowPrivate
{
    GtkWindow *parent;
...
G_DEFINE_TYPE_WITH_PRIVATE(ImportSpedizioniWindow, import_spedizioni_window, GTK_TYPE_DIALOG)
...
static void import_spedizioni_window_init (ImportSpedizioniWindow *win)
{
    ImportSpedizioniWindowPrivate *priv = import_spedizioni_window_get_instance_private (win);
...
static void import_spedizioni_window_dispose (GObject *object)
{
    ImportSpedizioniWindow *win = IMPORT_SPEDIZIONI_WINDOW (object);
    ImportSpedizioniWindowPrivate *priv = import_spedizioni_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (import_spedizioni_window_parent_class)->dispose (object);
}

static void import_spedizioni_window_finalize (GObject *object)
{
    ImportSpedizioniWindow *win = IMPORT_SPEDIZIONI_WINDOW (object);
    ImportSpedizioniWindowPrivate *priv = import_spedizioni_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (import_spedizioni_window_parent_class)->finalize (object);
}

static void import_spedizioni_window_class_init (ImportSpedizioniWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = import_spedizioni_window_dispose;
    G_OBJECT_CLASS (class)->finalize = import_spedizioni_window_finalize;
...
ImportSpedizioniWindow * import_spedizioni_window_new (GtkWindow *parent)
{
    ...
    ImportSpedizioniWindow *win = g_object_new (IMPORT_SPEDIZIONI_WINDOW_TYPE, "transient-for", parent, "use-header-bar", TRUE, NULL);
    ImportSpedizioniWindowPrivate *priv = import_spedizioni_window_get_instance_private (win);
