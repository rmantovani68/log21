...
#include "mainapp.h"
#include "mainwin.h"
#include "insert_distinta.h"
#include "mainfun.h"

#define WINDOW_NAME "insert-distinta-window"

static GtkWindow *_parent;

struct _InsertDistintaWindow
{
    GtkDialog parent;
};

typedef struct _InsertDistintaWindowPrivate InsertDistintaWindowPrivate;

struct _InsertDistintaWindowPrivate
{
    GtkWindow *parent;
...
G_DEFINE_TYPE_WITH_PRIVATE(InsertDistintaWindow, insert_distinta_window, GTK_TYPE_DIALOG)
...
static void insert_distinta_window_init (InsertDistintaWindow *win)
{
    InsertDistintaWindowPrivate *priv = insert_distinta_window_get_instance_private (win);
...
static void insert_distinta_window_dispose (GObject *object)
{
    InsertDistintaWindow *win = INSERT_DISTINTA_WINDOW (object);
    InsertDistintaWindowPrivate *priv = insert_distinta_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (insert_distinta_window_parent_class)->dispose (object);
}

static void insert_distinta_window_finalize (GObject *object)
{
    InsertDistintaWindow *win = INSERT_DISTINTA_WINDOW (object);
    InsertDistintaWindowPrivate *priv = insert_distinta_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (insert_distinta_window_parent_class)->finalize (object);
}

static void insert_distinta_window_class_init (InsertDistintaWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = insert_distinta_window_dispose;
    G_OBJECT_CLASS (class)->finalize = insert_distinta_window_finalize;
...
InsertDistintaWindow * insert_distinta_window_new (GtkWindow *parent)
{
    ...
    InsertDistintaWindow *win = g_object_new (INSERT_DISTINTA_WINDOW_TYPE, "transient-for", parent, "use-header-bar", TRUE, NULL);
    InsertDistintaWindowPrivate *priv = insert_distinta_window_get_instance_private (win);
