...
ep_bool_t DatiVettorePL_PF(PDATIORDINE pDatiOrdine,char *szCDVET,int nPos1,int nPos2,int nPos3)
{
	...
	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select elcdsvet,elcinvet,elclovet from ttve where elccdsoc='%s' and elccdmag='%s' and elccdvet='%s';",
		...
		szCDVET);
	if(DBntuples(DBRes)){
		InitData("LastPage",DBgetvalue(DBRes,0,0),PLPos_A4[nPos1].nSR,PLPos_A4[nPos1].nSC,FALSE);
		InitData("LastPage",DBgetvalue(DBRes,0,1),PLPos_A4[nPos2].nSR,PLPos_A4[nPos2].nSC,FALSE);
		InitData("LastPage",DBgetvalue(DBRes,0,2),PLPos_A4[nPos3].nSR,PLPos_A4[nPos3].nSC,FALSE);
	} else {
#ifdef TRACE
...
/*
*
* StampaPL_PF(PDATIORDINE pDatiOrdine,PDATICOLLO pDatiCollo,ep_bool_t bStorico)
*
...
ep_bool_t StampaPL_PF(PDATIORDINE pDatiOrdine,PDATICOLLO pDatiCollo,ep_bool_t bStorico)
{
	...
	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select sum(rp.rpqtspe),sum(rp.rpqtspe * ra.%s) from %s rp,%s ra where rp.rpcdpro=ra.racdpro and ra.ordprog=rp.ordprog and rp.ordprog='%s' and rp.rpnmcol=%d;",szValueField,szRigProd,szRicArt,pDatiOrdine->szORDPROG,pDatiCollo->nCPNMCOL);
	if(DBntuples(DBRes)){
	...
	/* righe di produzione */
	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select rpcdpro,sum(rpqtspe) from %s where ordprog='%s' and rpnmcol=%d group by rpcdpro;",szRigProd,pDatiOrdine->szORDPROG,pDatiCollo->nCPNMCOL);
...
int CreatePackingListColloPrefincato(char *pszPrinterName,char *szOrdProg,int nCollo,char *szFile,ep_bool_t bStorico)
{
...
#ifdef TRACE
	trace_debug(TRUE, TRUE, "Stampa Packing List Prefincato [%s,%d] Storico : %d",szOrdProg,nCollo,bStorico);
#endif
	...
	if(GetDatiOrdine(szOrdProg,&DatiOrdine,bStorico) && GetDatiCollo (szOrdProg, nCollo,&DatiCollo, bStorico) ){
		...
		nRC=1;
#ifdef TRACE
		trace_debug(TRUE, TRUE, "Collo [%s,%d] non presente",szOrdProg,nCollo);
#endif
	}
	return nRC;
}
...
ep_bool_t StampaPackingListColloPrefincato(char *pszPrinterName,char *szOrdProg,int nCollo,ep_bool_t bStorico)
{
	...
	if(bOK){
		LocalPrintFile(szPrintFile,pszPrinterName,NULL,FALSE,0);
	}
	return bOK;
}
...
ep_bool_t StampaPackingListBollaPrefincato(char *pszPrinterName,char *szOrdProg)
{
	...
	*/
	...
	//	PrintFile(szPrintFile,pszPrinterName,NULL,FALSE,0);
	//	unlink(szFileName);
