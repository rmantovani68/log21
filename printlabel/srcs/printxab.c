...
ep_bool_t DatiVettore(PDATIORDINE pDatiOrdine,char *szCDVET,int nPos1,int nPos2,int nPos3)
{
	...
	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select elcdsvet,elcinvet,elclovet from ttve where elccdsoc='%s' and elccdmag='%s' and elccdvet='%s';",
		...
		szCDVET);
	if(DBntuples(DBRes)){
		InitData("LastPage",DBgetvalue(DBRes,0,0),XABPos[nPos1].nSR,XABPos[nPos1].nSC,FALSE);
		InitData("LastPage",DBgetvalue(DBRes,0,1),XABPos[nPos2].nSR,XABPos[nPos2].nSC,FALSE);
		InitData("LastPage",DBgetvalue(DBRes,0,2),XABPos[nPos3].nSR,XABPos[nPos3].nSC,FALSE);
	} else {
#ifdef TRACE
...
ep_bool_t StampaXAB(PDATIORDINE pDatiOrdine,ep_bool_t bStorico)
{
	...
	*/
	...
	/* righe di produzione */
	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select rpcdpro,sum(rpqtspe) from %s where ordprog='%s' group by rpcdpro;",szRigProd,pDatiOrdine->szORDPROG);
	if((nTuples=DBntuples(DBRes))){
		for(nIndex=0;nIndex<nTuples;nIndex++){
			...
			sprintf(szBuffer,"%7d   ",    nRDBTSPE); strcat(szRiga,szBuffer);
	...
	*/
	if(bOK && !bRistampa){
		DBRes=DBExecQuery(Cfg.nDebugLevel>2,"update %s set roprxab=%d,roaaxab=%d,rodtxab='%s',rostato='%c' where ordprog='%s';",
			szRicOrd,
...
#ifdef TRACE
	trace_debug(TRUE, TRUE, "Stampa XAB Linea [%s - %s - %s]",szNMCED,szTPSPE,szCDLIN);
#endif
	sprintf(szPrintFile,"%s/%s.xab",Cfg.szPathStampe,pszPrinterName);

	DBRes=DBExecQuery(Cfg.nDebugLevel>2,"select ordprog from %s where ordtipo='%s' and ronmced='%s' and rotpspe='%s' and rocdlin='%s' order by ordprog;",szRicOrd,Cfg.szTipoOrdini,szNMCED,szTPSPE,szCDLIN);
	if((nTuples=DBntuples(DBRes))){
		/* Creazione del file di stampa */
		InitPrint(szPrintFile,MAX_RIGHE_XAB,MAX_COLONNE_XAB,Cfg.nPrimaRigaXAB,Cfg.nUltimaRigaXAB);
