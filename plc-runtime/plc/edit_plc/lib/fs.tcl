      ...
      if { $i != "" && [string range $i 0 0] != " " && [lindex $i 0] != "."} {
        ...
        if { [string first "<DIR>" $i] != -1 && [lindex $i 0] != ".."} {
          ...
          if { [string first ".plc" $i] != -1} {
            ...
            # Caso in cui non ci siano i nomi lunghi
            # ma la veccia gestione 8 + 3
            if { [lindex $i 1] == "plc" || [lindex $i 1] == "PLC" } {
              $listFile insert end \
               [string tolower [lindex $i 0]].[string tolower [lindex $i 1]]
            }
          }
        }
      }
