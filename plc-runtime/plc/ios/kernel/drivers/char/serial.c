...
#ifdef CONFIG_CNI_MULTIPORT
#ifndef CONFIG_NATIONAL_DUART_SUPPORT
#define CONFIG_NATIONAL_DUART_SUPPORT
#warning "CNI_MULTIPORT requires NATIONAL_DUART support"
#endif
#ifndef CONFIG_SERIAL_MANY_PORTS
#define CONFIG_SERIAL_MANY_PORTS
#warning "CNI_MULTIPORT requires MANY_PORTS support"
#endif
#ifndef CONFIG_SERIAL_MULTIPORT
#define CONFIG_SERIAL_MULTIPORT
#warning "CNI_MULTIPORT requires MULTIPORT support"
#endif
#endif /* CONFIG_CNI_MULTIPORT */
...
#if defined(CONFIG_NATIONAL_DUART_SUPPORT) || defined(CONFIG_AST_IRQ_FIX)
	" - CNi fix 1.2"
#endif
					;
...
#ifdef CONFIG_NATIONAL_DUART_SUPPORT
	  /* The NATIONAL NS16C55x is actually a 16550A. If I added
...
#ifdef CONFIG_CNI_MULTIPORT
static int cni_multiport_cfg_reg[NR_PORTS]
 = { 0, 0, 0, 0, 0x302, 0x302, 0x30A, 0x30A, /* ... */ };
static int cni_multiport_enabled[NR_IRQS] = { 0, /* ... */ };
static void cni_multiport_printsetup(void)
{
int limit,i;
char *dots,*tp;
char tmp[10*6+80];
	...
	for (i = 0, tp = tmp; i < limit; ++i) {
		sprintf(tp," 0x%03x",cni_multiport_cfg_reg[i]);
		tp += 6;
	}
	printk(KERN_INFO "cniser: ios={%s%s }\n",tmp,dots);
}
__initfunc(int cni_multiport_setup(char *str, int *ints))
{
int i,limit;
char *tmp;
	...
	if (!str) {
		if (ints[0] > NR_PORTS)
			...
			limit = ints[0];
		for (i = 1; i <= limit; ++i) {
			cni_multiport_cfg_reg[i-1] = ints[i];
		}
	} else if (!strncmp(str, "none", 4)) {
		for (i = 0; i < NR_PORTS; ++i) {
			cni_multiport_cfg_reg[i] = 0;
		}
	} else if (!strncmp(str, "port", 4)) {
		i = simple_strtoul(str+4, &tmp, 10);
		if (i < NR_PORTS && *tmp == '=') {
			++tmp;
			cni_multiport_cfg_reg[i] = simple_strtoul(tmp,NULL,0);
		}
		else {
			printk(KERN_INFO
			       "cni_multiport: too many ports, %s ignored.\n",
			       str);
		}
	}
	return 0;
...
static int cni_enable_multiport(struct serial_state * state)
{
int n = state -> line;
/* Allocazione della porta di configurazione e sua manipolazione... */
	if (cni_multiport_cfg_reg[n] != 0) {
	  if (! cni_multiport_enabled[state->irq]) {
	    cni_multiport_enabled[state->irq] = 1;
		if (check_region(cni_multiport_cfg_reg[n],1))
		  return -1;
		request_region(cni_multiport_cfg_reg[n],1,"serial(CNI)");
		outb((((state->port) >> 4) | 0x01), cni_multiport_cfg_reg[n]);
	  }
	}
	return 0;
}
static void cni_disable_multiport(struct serial_state * state)
{
int n = state -> line;
/* Allocazione della porta di configurazione e sua manipolazione... */
	if (cni_multiport_cfg_reg[n] != 0) {
	  if (cni_multiport_enabled[state->irq]) {
#if 0
	    cni_multiport_enabled[state->irq] = 0;
#endif
		release_region(cni_multiport_cfg_reg[n],1);
	  }
	}
}
#endif
...
/* Forza un nuovo interrupt creando un fronte di salita sulla linea,
 qualora fosse accaduto qualcosa durante l'elaborazione dell'interrupt
 corrente. Se non e` abilitato AST_IRQ_FIX, quest'operazione e`
 eseguita solo sulle schede CNi SER2. */
#if defined(CONFIG_CNI_MULTIPORT) && !defined(CONFIG_AST_IRQ_FIX)
	if (cni_multiport_cfg_reg[info->state->line])
#endif /* defined(CONFIG_CNI_MULTIPORT) && ! defined(CONFIG_AST_IRQ_FIX) */
#if defined(CONFIG_CNI_MULTIPORT) || defined(CONFIG_AST_IRQ_FIX)
	{
	  outb(0x00,multi->port1);
	/* Le due scritture successive servono solo a introdurre
	 un ritardo, che dovrebbe permettere al transistor che pilota
	 la l'IRQ ISA di uscire dalla saturazione (tutto questo
	 per risparmiare un condensatore!!!). */
	  outb(0x00,multi->port1);
	  outb(0x00,multi->port1);
	/* Riattivazione della linea. */
	  outb(0x80,multi->port1);
	}
#endif /* defined(CONFIG_CNI_MULTIPORT) || defined(CONFIG_AST_IRQ_FIX) */
#ifdef SERIAL_DEBUG_INTR
	printk("end.\n");
...
#ifdef CONFIG_CNI_MULTIPORT
static void ser_printk(struct serial_state * state)
{
char buf[64];

	buf[0] = '\0';
	if (state->flags & ASYNC_FOURPORT) {
		if (cni_multiport_cfg_reg[state->line])
			sprintf(buf," CNi-Ser2 (host = 0x%03x)",
				cni_multiport_cfg_reg[state->line]);
		else
			strcpy(buf," FourPort");
	}
	buf[sizeof(buf)-1] = '\0';
...
#ifdef CONFIG_NATIONAL_DUART_SUPPORT
	printk(" NATIONAL_DUART");
#define SERIAL_OPT
#endif
#ifdef CONFIG_AST_IRQ_FIX
	printk(" AST_IRQ_FIX");
#define SERIAL_OPT
#endif
#ifdef CONFIG_CNI_MULTIPORT
...
static void autoconfig(struct serial_state * state)
{
	...
	if (state->type == PORT_16550A) {
		...
		if (serial_in(info, UART_EFR) == 0) {
		   ...
		   * smarter. */
		  /* OK, there could be an ST16650 or a NATIONAL NS16C55x.
		   ...
		   * then verifying if it has really been set. */
			...
			if (serial_in(info, UART_EFR) & UART_EFR_CTS) {
			  ...
			  /* bit set unsuccesful. Perhaps we are using a NATIONAL double UART.
...
#else /* CONFIG_NATIONAL_DUART_SUPPORT */
			state->type = PORT_16650;
#endif /* CONFIG_NATIONAL_DUART_SUPPORT */
		} else {
			serial_outp(info, UART_LCR, 0xBF);
			if (serial_in(info, UART_EFR) == 0)
	...
	/* giusguerrini@racine.ra.it: Arrrgh! This messes up the NATIONAL
