...
static int open_ios(struct inode * inode, struct file * filp);
static int mmap_ios(struct file * file, struct vm_area_struct * vma);
static int ioctl_ios(struct inode *inode, struct file *file,
...
{
	outb_p(((memaddr >> 12) & 0xFE) | 0x01, ioaddr);
}
...
/* L'offset deve essere allineato alla pagina. */
	if (vma -> vm_offset & ~PAGE_MASK)
		return -ENXIO;

/* Rifiuta di mappare piu` di quanto competa alla scheda. */
	size = vma -> vm_end - vma -> vm_start;
	if (size + vma -> vm_offset > statics -> board[minor].tot_size)
		return -EINVAL;

	physaddr = vma -> vm_offset + statics -> board[minor].b.memaddr;
...
/* Ora si puo` procedere alla mappatura dell'indirizzo fisico. */
	if (remap_page_range(vma -> vm_start, physaddr, size, vma -> vm_page_prot))
		return -EAGAIN;
...
static void get_ios_ver(char *ver, volatile unsigned char *addr)
{
int j;
...
struct ios_board *b;
	...
	if (register_chrdev(IOSCNI_MAJOR,IOSCNI_NAME,&ios_fops)) {
		...
		/* Ricerca della versione del firmware. */
			get_ios_ver(ver[0],addr + IOSCNI_OFF_CPU1);
			get_ios_ver(ver[1],addr + IOSCNI_OFF_CPU2);
