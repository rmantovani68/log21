 ...
 */
struct canmodule_t {
    unsigned int id;                   /* ID del modulo. */
    canboard_t *cbr;                   /* Riferimento al bus contenitore. */
    ...
    char name[32];                     /* Dal registro 0x1008 */
    char hwver[32];                    /* Dal registro 0x1009 */
    char swver[32];                    /* Dal registro 0x100A */
    unsigned char state;               /* Stato del modulo: presente, non presente, non interrogato. */
    unsigned int aout_idx;             /* Indice delle uscite analogiche. */
    canpdo_t txpdo[CANOPEN_NUM_PDO];   /* Descrittori dei PDO associati al modulo. */
    canpdo_t rxpdo[CANOPEN_NUM_PDO];
    unsigned char n_ib;                /* Numero di byte nel PDO di input (lettura dizionario 0x6000:0). */
    unsigned int n_ibit;               /* Numero di bit, per comodita` */
    unsigned char n_ob;
    unsigned int n_obit;               /* Numero di bit, per comodita` */
    unsigned char n_ain;               /* Numero di ingressi e uscite analogiche. */
    unsigned char n_aout;
    unsigned char n_enc;               /* Numero ingressi encoder. */
    unsigned char in[32];              /* Stato ingressi. */
    unsigned char out[32];             /* Stato uscite. */
    unsigned short ain[16];            /* Stato ingressi analogici. */
    unsigned short aout[16];           /* Stato uscite analogiche. */
    long enc[16];                      /* Stato encoder. */
    sdo_rw_t sr;                       /* Stato dell'eventuale transazione SDO in corso. */
    sdo_t *sdo_req_head;               /* Lista delle richieste SDO inevase (testa e coda della lista). */
    sdo_t *sdo_req_tail;
};

typedef struct canmsgcb_t canmsgcb_t;

typedef void (*fn_cb)(canboard_t *cbr, canmsg_t *msg, void *arg);
...
typedef int (*open_fn_t)(canboard_t *cbr, int nch, int speed, int debug);
typedef int (*send_fn_t)(canboard_t *cbr, canmsg_t *msg);
typedef int (*recv_fn_t)(canboard_t *cbr, canmsg_t *msg);
...
struct canboard_t {
    ...
    int line;                                            /**< Linea CAN. */
    int fd_rx;                                           /**< File descriptor del canale CAN (sdoppiato per debug). */
    int fd_tx;
    int speed;                                           /**< Velocita` di linea. */
    int size_rdwr;                                       /**< A corredo (maledetto can4linux!): terzo parametro per read/write. */
    int pipe_mr[2];                                      /**< Pipe per comandare il ricevitore. */
    int pipe_rm[2];                                      /**< Pipe per risposte dal ricevitore. */
    int nfd;                                             /**< Numero di file descriptor gestiti (primo parametro di select). */
    int rx_sleeping;                                     /**< Flag di ricevitore dormiente. */
    pthread_t rx_thread;                                 /**< Thread ricevitore. */
    canmodule_t module[CANOPEN_NUM_ID];                  /**< Stato dei moduli. */
    int update_set[CANOPEN_NUM_PDO][UPDATE_SET_DIM];     /**< Insieme dei moduli/PDO che richiedono update. Usare ffs per la ricerca. */
    /* Opzioni di funzionamento varie: */
    int noremap_pdo_cniinf;                              /**< Flag per l'inibizione della mappatura dei PDO nei dispositivi della CNI Informatica. */
    canmsgcb_t msgcb[1 << CAN_ID_NBIT];                  /**< Tabella delle funzioni di reazione ai messaggi CAN entranti. */
    sdo_t sdo_pool[SDO_POOL_SIZE];                       /**< Pool delle richieste SDO libere (array preallocato e lista degli elementi non in uso). */
    sdo_t *sdo_pool_free;
    sdo_rw_t *sdo_rw_list;                               /**< Lista degli SDO asincroni in corso. */
    const canlldrv_t *lldrv;                             /**< Puntatori alle funzioni "driver" di basso livello. */
    void *llarg;                                         /**< Campo riservato alle funzioni di basso livello. */
};
...
/* secondo i vari tipi di controllore CAN (socketcan, simulato...) */
/**********************************************************************/
...
typedef struct _socketcan_t {
    int s;                     /* socket */
    int nch;
    pthread_t rx_thread;       /* Thread ricevitore. */
    int pipe_rx[2];           /* Pipe per l'accodamento dei messaggi ricevuti. */
} socketcan_t;
...
/**********************************************************************/
/* Parte generica indipendente dal can device                         */
/**********************************************************************/
...
#endif
                rv = canboard_read_dictionary(cbr, 1, index, 0, &d, sizeof(d));
                if (rv == 0) {
                    if (cbr -> debug & 4) {
...
static int sdo_rw_start(sdo_rw_t *sr, canmodule_t *cm, int idx, int subidx, void *data, unsigned int *len, int wr)
{
...
static int sdo_async_start(sdo_t *sdo)
{
    ...
    sr = &sdo -> cm -> sr;
    sdo -> sr = sr;
    rv = sdo_rw_start(sr, sdo -> cm, sdo -> idx, sdo -> subidx, sdo -> data, &sdo -> len, sdo -> wr);
...
sdo_t *canboard_async_sdo(canboard_t *cbr, int mod,int idx, int subidx, void *data, unsigned int len, int wr, sdo_cb_t cb, void *cb_arg)
{
