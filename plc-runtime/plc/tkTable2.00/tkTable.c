...
static void	TableFlushCache _ANSI_ARGS_((Table *tablePtr));
static GC	TableGetGc _ANSI_ARGS_((Table *tablePtr, TableTag *tagPtr));
...
#define CMD_GET		11	/* get mode a la listbox */
#define	CMD_HEIGHT	12	/* (re)set row heights */
#define CMD_ICURSOR	13	/* set the insertion cursor */
#define CMD_INDEX	15	/* get an index */
#define CMD_INSERT	16	/* insert text at any position */
#define	CMD_REREAD	17	/* reread the current selection */
#define CMD_SCAN	18	/* scan command a la listbox */
#define CMD_SEE		19	/* see command a la listbox */
#define CMD_SELECTION	21	/* selection command a la listbox */
#define CMD_SET		22	/* set command, to set multiple items */
#define	CMD_TAG		23	/* tag command menu */
#define CMD_VALIDATE	24	/* validate contents of active cell */
#define CMD_VERSION	25	/* hidden command to return version */
#define	CMD_WIDTH	26	/* (re)set column widths */
#define	CMD_WINDOW	27	/* manage embedded windows */
#define CMD_XVIEW	28	/* change x view of widget (for scrollbars) */
#define CMD_YVIEW	29	/* change y view of widget (for scrollbars) */
 ...
 */
static GC
TableGetGc(Table *tablePtr, TableTag *tagPtr)
{
  XGCValues gcValues;
...
TableDisplay(ClientData clientdata)
{
  ...
  int rowFrom, rowTo, colFrom, colTo,
    ...
    numChars, new, boundW, boundH, maxW, maxH,
  ...
  invalidWidth = MIN(tablePtr->invalidWidth, boundW-invalidX);
  invalidHeight = MIN(tablePtr->invalidHeight, boundH-invalidY);
  ...
  for (row = rowFrom; row <= rowTo; row++) {
    ...
    rowPtr = FindRowColTag(tablePtr, urow, SEL_ROW);
	...
	y -= invalidY;
      ...
      if (new) {
	colPtr = FindRowColTag(tablePtr, ucol, SEL_COL);
	...
	textLayout = Tk_ComputeTextLayout(tagPtr->fontPtr, string, numChars,
					  tagPtr->wrap ? width : 0,
					  tagPtr->justify, 0, &itemW, &itemH);
	 ...
	 */
	if (activeCell) {
	  Tk_CharBbox(textLayout, MIN(numChars, tablePtr->icursor),
		      &cx, &cy, &cw, &ch);
		...
		int width, int height, int flags)
{
  ...
  if ((x > Tk_Width(tkwin)) || (y > Tk_Height(tkwin)))
    return;
  ...
  if (tablePtr->flags & REDRAW_PENDING) {
    ...
    if ((flags & INV_FORCE) && !(tablePtr->batchMode)) {
...
{
  Table *tablePtr = (Table *) clientData;
  int x, y, width, height, row, col, update = 1;
  ...
  if (update && Tk_IsMapped(tablePtr->tkwin) &&
...
TableCursorEvent(ClientData clientData)
{
  ...
  /* invalidate the cell */
  if (TableCellVCoords(tablePtr, tablePtr->activeRow, tablePtr->activeCol,
		       &x, &y, &width, &height, 0))
    TableInvalidate(tablePtr, x, y, width, height, INV_FORCE);
}
  ...
  if (tablePtr->exportSelection) {
	...
	entryPtr != NULL; entryPtr = Tcl_NextHashEntry(&search)) {
      TableParseArrayIndex(&row, &col,
			   Tcl_GetHashKey(tablePtr->selCells,entryPtr));
      Tcl_DeleteHashEntry(entryPtr);
      TableCellVCoords(tablePtr, row-tablePtr->rowOffset,
		      col-tablePtr->colOffset, &x, &y, &width, &height, 0);
      TableInvalidate(tablePtr, x, y, width, height, 0);
    }
  }
     ...
     char **argv;			/* Argument strings. */
{
  ...
  case CMD_ACTIVATE:
	...
	if (!(tablePtr->flags & BROWSE_CMD) && tablePtr->browseCmd != NULL) {
      ...
      } else if (!(tablePtr->flags & ACTIVE_DISABLED) && argv[2][0] == '@' &&
  ...
  case CMD_CURVALUE:
	...
	if (TableCellVCoords(tablePtr, tablePtr->activeRow,
			     tablePtr->activeCol, &x, &y, &width, &height, 0))
	  TableInvalidate(tablePtr, x, y, width, height, INV_FORCE);
	if (key == TCL_ERROR) {
	  result = TCL_ERROR;
	  break;
     ...
     XEvent *eventPtr;		/* Information about event. */
{
  ...
  case FocusIn:
    if (eventPtr->xfocus.detail != NotifyInferior) {
      tablePtr->flags |= (HAS_FOCUS|REDRAW_BORDER);
      TableRedrawHighlight(tablePtr);
      /* Turn the cursor on */
      TableConfigCursor(tablePtr);
    }
...
/*  ANDREA  */
  Tcl_SetVar(interp, "tbl_version", TBL_VERSION, TCL_GLOBAL_ONLY);
/* */
