...
static Tk_CustomOption tagStateTypeOpt = { Cmd_OptionSet, Cmd_OptionGet,
...
static Tk_ConfigSpec tagConfig[] = {
  {TK_CONFIG_ANCHOR, "-anchor", "anchor", "Anchor", NULL,
   Tk_Offset(TableTag, anchor), 0 },
  {TK_CONFIG_BORDER, "-background", "background", "Background", NULL,
   Tk_Offset(TableTag, bgBorder), TK_CONFIG_NULL_OK },
  {TK_CONFIG_SYNONYM, "-bg", "background", (char *) NULL,
   (char *) NULL, 0, 0 },
  {TK_CONFIG_BORDER, "-foreground", "foreground", "Foreground", NULL,
   Tk_Offset(TableTag, foreground), TK_CONFIG_NULL_OK },
  {TK_CONFIG_SYNONYM, "-fg", "foreground", (char *) NULL,
   (char *) NULL, 0, 0 },
  {TK_CONFIG_FONT, "-font", "font", "Font", NULL,
   Tk_Offset(TableTag, fontPtr), TK_CONFIG_NULL_OK },
  {TK_CONFIG_JUSTIFY, "-justify", "justify", "Justify", "left",
   Tk_Offset(TableTag, justify), 0 },
  {TK_CONFIG_STRING, "-image", "image", "Image", "",
   Tk_Offset(TableTag, imageStr), TK_CONFIG_NULL_OK},
  {TK_CONFIG_RELIEF, "-relief", "relief", "Relief", NULL,
   Tk_Offset(TableTag, relief), TK_CONFIG_NULL_OK },
  {TK_CONFIG_BOOLEAN, "-showtext", "showText", "ShowText", "0",
   Tk_Offset(TableTag, showtext), 0 },
  {TK_CONFIG_CUSTOM, "-state", "state", "State", "unknown",
   Tk_Offset(TableTag, state), 0, &tagStateTypeOpt},
  {TK_CONFIG_BOOLEAN, "-wrap", "wrap", "Wrap", "0",
   Tk_Offset(TableTag, wrap), 0 },
...
{
  if (addTag->anchor != (Tk_Anchor)-1)
    baseTag->anchor = addTag->anchor;
  if (addTag->bgBorder != NULL)
    baseTag->bgBorder = addTag->bgBorder;
  if (addTag->foreground != NULL)
    baseTag->foreground = addTag->foreground;
  if (addTag->fontPtr != NULL)
  ...
  if (addTag->state != STATE_UNKNOWN)
    baseTag->state = addTag->state;
  if (addTag->justify != (Tk_Justify)-1)
 ...
 */
TableTag *
FindRowColTag(Table *tablePtr, int cell, int type)
{
  Tcl_HashTable *hash;
  Tcl_HashEntry *entryPtr;

  hash = (type == SEL_ROW) ? tablePtr->rowStyles : tablePtr->colStyles;
  if ((entryPtr = Tcl_FindHashEntry(hash, (char *)cell)) == NULL) {
    char *cmd = (type == SEL_ROW) ? tablePtr->rowTagCmd : tablePtr->colTagCmd;
    if (cmd) {
	    ...
	    int argc, char *argv[])
{
  ...
  case TAG_INCLUDES:
    ...
    if ((scanPtr && ((TableTag *) Tcl_GetHashValue(scanPtr) == tagPtr)) ||
        (tagPtr == FindRowColTag(tablePtr, row, SEL_ROW)) ||
        (tagPtr == FindRowColTag(tablePtr, col, SEL_COL))) {
