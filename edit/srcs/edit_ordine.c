...
#define WINDOW_NAME "edit-ordine-window"

struct _EditOrdineWindow
{
    GtkDialog parent;
};

typedef struct _EditOrdineWindowPrivate EditOrdineWindowPrivate;

struct _EditOrdineWindowPrivate
{
    GtkWidget *lb_msg;
    GtkWidget *sw_list;
...
G_DEFINE_TYPE_WITH_PRIVATE(EditOrdineWindow, edit_ordine_window, GTK_TYPE_DIALOG)
...
static void edit_collo_activated (GSimpleAction *action, GVariant *parameter, gpointer win)
{
    EditOrdineWindowPrivate *priv = edit_ordine_window_get_instance_private (win);

    EditColloWindow *w;

    w = edit_collo_window_new (EDIT_ORDINE_WINDOW (win));
...
static GActionEntry entries[] = {
  ...
  {"edit_collo", edit_collo_activated, NULL, NULL, NULL},
...
static void edit_ordine_window_init (EditOrdineWindow *win)
{
    EditOrdineWindowPrivate *priv = edit_ordine_window_get_instance_private (win);
...
static void edit_ordine_window_dispose (GObject *object)
{
    EditOrdineWindow *win = EDIT_ORDINE_WINDOW (object);
    EditOrdineWindowPrivate *priv = edit_ordine_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (edit_ordine_window_parent_class)->dispose (object);
}

static void edit_ordine_window_class_init (EditOrdineWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = edit_ordine_window_dispose;
    ...
    gtk_widget_class_bind_template_child_private (GTK_WIDGET_CLASS (class), EditOrdineWindow, sw_list      );
}

EditOrdineWindow * edit_ordine_window_new (MainWindow *win)
{
    return g_object_new (EDIT_ORDINE_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);
}
