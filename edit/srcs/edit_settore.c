...
#define WINDOW_NAME "edit-settore-window"

struct _EditSettoreWindow
{
    GtkDialog parent;
};

typedef struct _EditSettoreWindowPrivate EditSettoreWindowPrivate;

struct _EditSettoreWindowPrivate
{
    GtkWidget *lb_msg;
    GtkWidget *sw_list;
...
G_DEFINE_TYPE_WITH_PRIVATE(EditSettoreWindow, edit_settore_window, GTK_TYPE_DIALOG)
...
{
    EditSettoreWindowPrivate *priv = edit_settore_window_get_instance_private (win);
}
...
static void edit_settore_window_init (EditSettoreWindow *win)
{
    EditSettoreWindowPrivate *priv = edit_settore_window_get_instance_private (win);
...
static void edit_settore_window_dispose (GObject *object)
{
    EditSettoreWindow *win = EDIT_SETTORE_WINDOW (object);
    EditSettoreWindowPrivate *priv = edit_settore_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (edit_settore_window_parent_class)->dispose (object);
}

static void edit_settore_window_class_init (EditSettoreWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = edit_settore_window_dispose;
...
{
    return g_object_new (EDIT_SETTORE_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);
}
