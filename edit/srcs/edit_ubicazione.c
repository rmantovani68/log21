...
#define WINDOW_NAME "edit-ubicazione-window"

struct _EditUbicazioneWindow
{
    GtkDialog parent;
};

typedef struct _EditUbicazioneWindowPrivate EditUbicazioneWindowPrivate;

struct _EditUbicazioneWindowPrivate
{
    GtkWidget *lb_msg;
    GtkWidget *sw_list;
...
G_DEFINE_TYPE_WITH_PRIVATE(EditUbicazioneWindow, edit_ubicazione_window, GTK_TYPE_DIALOG)
...
{
    EditUbicazioneWindowPrivate *priv = edit_ubicazione_window_get_instance_private (win);
}
...
static void edit_ubicazione_window_init (EditUbicazioneWindow *win)
{
    EditUbicazioneWindowPrivate *priv = edit_ubicazione_window_get_instance_private (win);
...
static void edit_ubicazione_window_dispose (GObject *object)
{
    EditUbicazioneWindow *win = EDIT_UBICAZIONE_WINDOW (object);
    EditUbicazioneWindowPrivate *priv = edit_ubicazione_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (edit_ubicazione_window_parent_class)->dispose (object);
}

static void edit_ubicazione_window_class_init (EditUbicazioneWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = edit_ubicazione_window_dispose;
...
{
    return g_object_new (EDIT_UBICAZIONE_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);
}
