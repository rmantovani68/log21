...
#define WINDOW_NAME "edit-imballo-window"

struct _EditImballoWindow
{
    GtkDialog parent;
};

typedef struct _EditImballoWindowPrivate EditImballoWindowPrivate;

struct _EditImballoWindowPrivate
{
    GtkWidget *lb_msg;
    GtkWidget *sw_list;
...
G_DEFINE_TYPE_WITH_PRIVATE(EditImballoWindow, edit_imballo_window, GTK_TYPE_DIALOG)
...
{
    EditImballoWindowPrivate *priv = edit_imballo_window_get_instance_private (win);
}
...
static void edit_imballo_window_init (EditImballoWindow *win)
{
    EditImballoWindowPrivate *priv = edit_imballo_window_get_instance_private (win);
...
static void edit_imballo_window_dispose (GObject *object)
{
    EditImballoWindow *win = EDIT_IMBALLO_WINDOW (object);
    EditImballoWindowPrivate *priv = edit_imballo_window_get_instance_private (win);
    ...
    G_OBJECT_CLASS (edit_imballo_window_parent_class)->dispose (object);
}

static void edit_imballo_window_class_init (EditImballoWindowClass *class)
{
    G_OBJECT_CLASS (class)->dispose = edit_imballo_window_dispose;
...
{
    return g_object_new (EDIT_IMBALLO_WINDOW_TYPE, "transient-for", win, "use-header-bar", TRUE, NULL);
}
